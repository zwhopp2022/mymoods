...
public func determineWeek() -> (Date, Date) {
    let calendar = Calendar.current
    var startDate = Date()
...
public func createWeeklyPieChartSlicesFromMoods() -> ([PiePart], Int) {
    ...
    let calendar = Calendar.current
    let startDay = calendar.component(.day, from: currentWeekInterval.0)
    let startMonth = calendar.component(.month, from: currentWeekInterval.0)
    let endMonth = calendar.component(.month, from: currentWeekInterval.1)
    let endDay = calendar.component(.day, from: currentWeekInterval.1)
    ...
    for input in feelingsToExamine {
        if (((input.numDay >= startDay) && (input.numDay <= endDay))) && ((input.numMonth == startMonth) || (input.numMonth == endMonth))
        {
            switch input.mood {
            case "Saddest":
                pieSlices[0].countOfMood += 1
...
public func createMonthlyPieChartSlicesFromMoods() -> ([PiePart], Int) {
    ...
    for input in feelingsToExamine {
        if (input.numMonth == currentMonth)
        {
            switch input.mood {
            case "Saddest":
                pieSlices[0].countOfMood += 1
...
struct WeeklyPieChart: View {
    
    @State private var animate = false
    @State private var moodSlices = createWeeklyPieChartSlicesFromMoods().0
    @State private var moodsCount = createWeeklyPieChartSlicesFromMoods().1
